...
st.title("📊 인정평가 부적합 분석(ISO/IEC 17021-1 기반)")
...
def drop_noise_columns(df):
    """기관명/평가종류/시작일/종료일/발행번호 및 Unnamed:* 제거"""
...
def add_req_text(findings, standards):
    """조항/세부조항 기준으로 요구사항 붙이기 (컬럼 안전검사 포함)"""
    if standards is None:
        return findings
    # 가능한 컬럼명 후보 탐색
    std_cols = [c for c in standards.columns]
    ...
    def match_req(row):
        ...
        if sub in std[clause_col].values:
            return std.loc[std[clause_col] == sub, req_col].values[0]
        elif main in std[clause_col].values:
            return std.loc[std[clause_col] == main, req_col].values[0]
        return ""
...
show_cols = [c for c in ["조항","세부조항","구분","요구사항","내용"] if c in df.columns]
...
ISO 9001 심사 실행 지침 등 ISO Auditing Practices Group 문서에서도 이 정의를 사용하며, 부적합을 문서화할 때는 “요구사항 위반 → 객관적 증거 → 원인 → 시정 조치 → 재발 방지 계획” 등의 체계가 필요하다고 안내합니다. 
심사 표준인 ISO 19011:2018에서는 “audit findings (심사 결과)” 개념을 다루며, 심사 결과는 기준(criterion) 대비 현실(실제 상태)을 평가한 결과로서, conformity 또는 nonconformity 가 될 수 있다고 규정합니다. 
따라서 ISO 체계에서 부적합은 “심사 기준(표준, 규정, 내부 문서 등) 중 하나 이상의 요구사항을 충족하지 못한 상태”로 간주됩니다.
...
심사 증거 (audit evidence)
요구사항 (requirement)
부적합 진술 (statement of nonconformity) 
...
| **검증 및 종결** | 평가사(심사원)이 조치 유효성을 검증 후 종결 | ISO/APG |
""")
...
부적합과 권고사항(또는 “개선 기회, Observation / OFI”)은 평가 및 관리 시스템 운영 시 자주 혼동되는 개념이므로, 구별 기준을 명확히 두는 것이 중요합니다.

아래 기준들을 중심으로 설명드립니다.

(1) ISO 규격 관점: 권고사항/개선기회는 정의된 용어가 아님

ISO 9000 / ISO 19011 / ISO 17021 등의 표준에는 “Opportunity for Improvement / Recommendation” 이라는 용어의 공식 정의는 없습니다. 심사 지침에서는 심사원이 부적합을 지적으로 할 수 없는 경우라도 시스템 개선 가능성 등에 대해 의견을 제시할 수 있음을 인정하지만, 권고사항은 의무 조치 항목은 아니다 라는 점이 강조됩니다. 

ISO/IEC 17021-1:2015 (인증기관에 대한 요구사항)에서는 관찰(observation) 또는 개선 기회가 식별되고 기록될 수 있으나, 평사 결과가 nonconformity인 경우에는 그것을 개선 기회로 기록해서는 안 된다고 정의하고 있으며, 개선 기회는 기준 위반의 증거가 없는 영역에서 제시 가능한 조언 수준으로 다루어 지고 있습니다.
...
즉, ISO 체계에서는 권고사항 또는 개선기회는 심사인이 제안할 수 있는 개선 아이디어 수준이며, 부적합과는 별도로 취급됩니다.
